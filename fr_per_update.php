...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if ($valid) { // if valid user input update the database
		...
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$sql = "UPDATE fr_persons  set fname = ?, lname = ?, email = ?, mobile = ?, password = ?, title = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ?,address = ?,city = ?,state = ?,zip = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
				...
				<div class="control-group">
					...
					<div class="controls">
						<select class="form-control" name="title">
							...
							if (0==strcmp($_SESSION['fr_person_title'],'Customer')) echo '<option selected value="Volunteer" >Customer</option>';
							...
							'<option value="Volunteer">Customer</option>
